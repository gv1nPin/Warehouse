...
from warehouse.api.mappers.shipment import ShipmentMapper
...
class ShipmentReceiptService(AbstractShipmentReceiptService):
    def __init__(self, uow):
        # зависит только от Unit of Work для атомарных транзакций
        self.uow = uow
    ...
    def get_incoming(self, employee_id: int) -> List[StageDTO]:
        ...
        raw_stages = self.uow.receipt.get_incoming_stages_by_warehouse(
            warehouse_id=employee["warehouse_id"], 
            status_id=constants.SHIPPED
        )
        return [ShipmentMapper.to_stage_dto(stage) for stage in raw_stages]
    ...
    def enter_actual_quantity(self, employee_id: int, item_id: int, quantity: float, comment: Optional[str] = None) -> None:
        ...
        with self.uow:
            item = self.uow.receipt.get_stage_item_by_id(item_id)
            if not item:
                raise EntityNotFoundException("Позиция этапа не найдена.")
                
            stage = self.uow.receipt.get_stage_by_id(item["stage_id"])
            ...
            self.uow.receipt.set_actual_quantity(item_id, quantity, comment)
    ...
    def check_discrepancies(self, employee_id: int, stage_id: int) -> List[DiscrepancyDTO]:
        """Возвращает позиции без факта и позиции с несовпадением. Ничего не меняет в БД."""
        ...
        for item in stage_items:
            if item["actual_quantity"] is None or float(item["actual_quantity"]) != float(item["document_quantity"]):
                discrepancies.append(ShipmentMapper.to_discrepancy_dto(item))
    ...
    def accept_stage(self, employee_id: int, stage_id: int) -> StageDTO:
        ...
        with self.uow:  
            ...
            if employee["warehouse_id"] != stage["to_warehouse_id"]:
                failed_audit = ShipmentMapper.to_operation_history_data(
            ...
            # Определяем наличие расхождений
            ...
            # 1. Завершаем текущий этап (сохраняем acceptor_id и received_at)
            ...
            # 2. Начисляем остатки на склад получателя (для позиций, где факт > 0)
            for item in stage_items:
            ...
            # Логируем действие в историю операций
            ...
            # 3. ВНУТРЕННЯЯ ЛОГИКА ТРАНЗИТА (Вместо координатора)
            # Ищем, существует ли следующий этап в рамках этой поставки (порядковый номер + 1)
            ...
            if next_stage:
                # Если цепочка маршрута продолжается (Кросс-докинг / Транзит)
                logging.info(f"Обнаружен следующий этап ID {next_stage['id']}. Переводим его в режим подготовки.")
                
                # Автоматически переводим следующий этап в статус обработки / ожидания отправки
                self.uow.receipt.update_stage_status(next_stage["id"], status_id=constants.PROCESSING)
                
                # При необходимости можно скопировать принятые товары в качестве документального количества следующего шага:
                for item in stage_items:
                    self.uow.receipt.update_next_stage_item_document_quantity(
                        next_stage_id=next_stage["id"],
                        product_id=item["product_id"],
                        new_document_quantity=float(item["actual_quantity"])
                    )
            else:
                # Если это был завершающий этап маршрута — закрываем всю поставку (Shipment)
            ...
            # Возвращаем актуальное состояние измененного этапа
            updated_stage = self.uow.receipt.get_stage_by_id(stage_id)
            return ShipmentMapper.to_stage_dto(updated_stage)
